...
def breadthFirstSearch(problem: SearchProblem) -> List[Directions]:
    ...
    frontier = util.Queue()
    frontier.push((problem.getStartState(), []))
    visited = [] # Note: sometimes in CS188 a list is used if states are unhashable, but pacman states are tuples. Let's use list to be safe or set for speed.
    visited_set = set() # we will use set for O(1) lookups
